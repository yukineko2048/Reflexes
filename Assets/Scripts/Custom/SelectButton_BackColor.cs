...
public class SelectButton_BackColor : MonoBehaviour, ISelectButton_BackColor
{
    ...
    public void InstantiateButton()
    {
        ...
        for (int i = 0; i < this._color.Length; ++i)
        {
            ...
            button.transform.Find("SelectButton").GetComponent<Image>().color = this._color[i];
            selected[i] = save.backColor[i].selected;
            unlock[i] = save.backColor[i].unlock;
            name[i] = save.backColor[i].name;
            sellingPrice[i] = save.backColor[i].sellingPrice;
        }
